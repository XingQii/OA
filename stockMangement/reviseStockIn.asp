...
<%
requestExist request("stockInId")
Dim rs
set rs=Server.CreateObject("ADODB.Recordset")
rs.open "select * from stockIn where id="&request("stockInId"),conn,1,3
recordSetExist rs
If Request("action")="revise" Then
  ...
  rs("stockInDate")=Request.Form("stockInDate")
...
%>
  ...
  <li><a href="#">投入品入库</a></li>
    ...
    <form action="?action=revise&stockInId=<%=request("stockInId")%>" method="post">
      <ul class="forminfo">
        ...
        <li>
          <label for="name">名称</label><input  type="text" name="name" id="name" value="<%=rs("name")%>" required>
        </li>
        <li>
        <label for="stockInDate">入库日期</label>
          <input  type="date" name="stockInDate" id="stockInDate" value="<%=rs("stockInDate")%>" required>
        </li>
        <li>
