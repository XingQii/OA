...
requestExist request("shipId")
 ...
 Dim rsProduct
  ...
  rsProduct.open "select * from product where id="&rs("MatchId"),conn,1,1
...
If Request("action")="revise" Then
  ...
  rs("receiverPhone")=Request.Form("receiverPhone")
    If rsProduct("id")<>rs("matchId") Then
    ...
    <form action="?action=revise&shipId=<%=request("shipId")%>" method="post">
      <ul class="forminfo">
        ...
        <label for="productNumber">产品编号</label>
          ...
          <input type="text" id="productPrompt" name="productPrompt" >
