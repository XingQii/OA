...
<%Dim rs,rsLevel
 set rs=Server.CreateObject("ADODB.Recordset")
  rs.open "select * from supplier order by id desc",conn,1,1
...
<table class="tablelist">
  ...
  <tbody>
   <%
    ...
    <td><%=rs("product")%></td>
    <td><%Select Case rs("whetherFunded")
    ...
    Case false response.Write "否"
    End Select%></td>
    <td><%getLevel rs("supplierLevel")%></td>
     <td>
       <a href="supplierDetail.asp?supplierId=<%=rs("id")%>">查看详情</a>
      <a href="updateSupplier.asp?supplierId=<%=rs("id")%>">修改</a>
