...
<table cellpadding="0" cellspacing="0" style="width:100%;height:445px;table-layout:fixed;">
 ...
 <%
  ...
  <%
  set URS=WS_S.HX_SetRSD("","HX_CompanyUser"," ")
	if URS.recordcount>0 then	  
	  do until URS.eof
	   response.write "<tr><td><input name='Uid"&aaa&"' type='hidden' value='"&urs("WS_Uid")&"'><input type='radio' name='name' value="&urs("WS_Name")&" id='WSOA"&aaa&"'><label for='WSOA"&urs("WS_Uid")&"'><font color=blue>"&URS("WS_Name")&"</font></label>&nbsp;&nbsp;</td></tr>"
	   URS.movenext
	   loop
	   end if 
     %>
